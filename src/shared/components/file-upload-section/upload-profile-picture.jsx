...
import { appointeeProfilePictureUpdateSuccess, appointeeProfilePictureUpdateFailure } from "shared/constants/constants";
import showErrorMessage from "shared/utils/associate/show-error-message";
import showSuccessMessage from "shared/utils/associate/show-success-message";

const ProfileImageUploader = ({ setSelectedImage, selectedImage, uploadedFileDetails, setUploadedFileDetails, uploadedFile, setUploadedFile, appointeeId, userId }) => {
    ...
    const checkIfImageIsBlack = (base64Image) => {
        return new Promise((resolve) => {
            ...
            img.onload = () => {
                ...
                // Check a sample of pixels to improve performance
                const step = 4 * 100; // Skip 100 pixels
                ...
                resolve(avgBrightness < 10); // Consider black if very dark
            };
    ...
    //     callApi(previewImage);
    //     setPreviewDialog(false);
    //     setOpenDialog(false);
    // };

    const confirmPreview = () => {
        storeFileDetails(selectedImage);  // Now store file details only when confirmed
        setPreviewDialog(false);
        setOpenDialog(false);
    };
    ...
    const storeFileDetails = (file) => {
        const fileArray = [new File([file], file.name, { type: file.type, lastModified: file.lastModified })];
        const fileDetails = [
            {
                uploadDetailsId: 0,
                fileName: file.name,
        ...
        <Stack spacing={2} alignItems="center">
            ...
            <Dialog open={confirmDialog} onClose={() => setConfirmDialog(false)}>
                ...
                <DialogActions>
                    ...
                    <Button variant="contained" onClick={confirmCapture}>Yes</Button>
                </DialogActions>
            </Dialog>
            <Dialog open={previewDialog} onClose={() => setPreviewDialog(false)}>
                ...
                <DialogActions>
                    <Button onClick={() => { setPreviewDialog(false); setOpenDialog(true); }}>Cancel</Button>
                    <Button variant="contained" onClick={confirmPreview}>Save</Button>
                </DialogActions>
            </Dialog>
        </Stack>
    );
