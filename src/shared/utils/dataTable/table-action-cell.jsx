...
export const TableActionCell = (props1, props2) => {
  ...
  const handleCloseUserDetails = (id) => {
    const dialogComponent = <Typography mt={2}>Do you want to delete the user?</Typography>;
    const confirmationModelContent = {
      dialogComponent,
    };
    ...
    actionList.map((action) => {
      return (
        <>
          ...
          ['MV', 'MRV', 'RD'].includes(verificationStatusCode) &&
          actionPermissionList &&
          actionPermissionList['A016'] ? (
            <RedirectFab
              onClick={() =>
                navigateTo(`${toMannualVerification}`, { state: verificationStatusCode })
              }
          ...
          {(action === 'DWNLDTRUSTPSSBK') & (rowAttribute.isTrustPFApplicable === true) ? (
            <DownLoadFab
