...
const ConsentModal = ({ open, consentModalContent }) => {
  ...
  const [isConsentProcessed, setIsConsentProcessed] = useState(
    appointeeStatusDetailsData[0]?.isConsentProcessed,
  );
  const { dialogTitle, dialogComponent, dialogContentText, closeConsentModal, consentCallBack } =
    consentModalContent || '';
    ...
    if (response) {
      ...
      if (responseInfo === 'Success') {
        ...
        setLocalStorageItem(
          ...
          updatedAppointeeStatusResponse?.responseInfo,
        );
  ...
  return (
    <Dialog fullWidth={true} maxWidth={'md'} open={open}>
      ...
      <DialogActions>
        {consentStatus !== 1 ? (
          <>
            <Button1 onClick={() => submitConsentStatus(1)}>Agree</Button1>
            {isConsentProcessed !== true && (
              <Button1 onClick={() => submitConsentStatus(2)}>Disagree</Button1>
            )}
          </>
        ) : null}
        {consentStatus === 1 ? (
          <Button1 onClick={() => submitConsentStatus(3)}>Revoke</Button1>
        ) : null}
        <Button1 onClick={closeConsentModal}>Close</Button1>
      </DialogActions>
    </Dialog>
  );
};
