...
const Modals = ({ closeModal }) => {
  ...
  // todo: move this code to another file start
  ...
  // todo: move this code to another file end
  const { pathname } = useLocation();
  // const setDropdownList = functionSlice && functionSlice[0] && functionSlice[0].setDropdownList;
  ...
  // ✅ Generic function to open/close modals dynamically
  ...
  // ✅ Confirmation Yes/No Modal Handlers
  ...
  const openConsentModal = useCallback((consentModalContent,
    consentCallBack) => {
  ...
  // ✅ Appointee Manual Verification Modal Handlers
  ...
  // ✅ Store functions in Redux if not already stored
  ...
  const openPassbookViewModel = useCallback((appointeeId, passbookDetails, passbookStatusCode) => {
    ...
    console.log('passbookDetails', passbookDetails);
  ...
  const openEmploymentViewModel = useCallback((appointeeId, userId, epfoDetails, passbookStatusCode) => {
    setAppointeeId(appointeeId);
    setUserId(userId);
  ...
  useEffect(() => {
    const updateFunctionSlice = (key, value) => {
      if (!functionSlice[0]?.[key]) { // Check if the property already exists
