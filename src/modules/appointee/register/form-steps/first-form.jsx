...
const FirstForm = ({
  ...
  const handlePassporNumbertHelp = () => {
    const passportHelpContent = {
      ...
      dialogContentComponent: (
        <img src={PassportSample} alt='Help' style={{ maxWidth: '100%', maxHeight: '100%' }} />
      ),
      maxWidth: 'sm',
      btnName: 'Close',
    };
      ...
      dialogContentText: (
        <>
          <Typography sx={subHeadingContentTextStyle}>
            {}
            This won't save the changes you have made. Do you want to proceed without saving
            changes?
          </Typography>
  ...
  const handleAppointeeFormPage1Save = async (formElement) => {
    ...
    if (passportAvailable === 'Y') {
      ...
      if (
        (firstPageForm?.nationality.toLowerCase() === 'indian' ||
            ...
            >
              {genderList &&
                genderList.map((gender, index) => {
                  const { name, icon, selected, id, code, selectGender } = gender;
              ...
              <CustomeDatePicker
                ...
                setValue={(newDate, name) => {
                  if (newDate) {
                    ...
                    handleFirstPageFormInputChange(null, 'dateOfBirth');
                  }
                }}
                required={true}
                disableFuture={true}
              ...
              <GridRow>
                <Grid item xs={12} md={6} sx={{ paddingLeft: '0px !important' }}>
                  <CustomeDatePicker
                    ...
                    value={
                      firstPageForm?.passportValidFrom
                        ? dayjs(firstPageForm?.passportValidFrom)
                        : null
                    }
