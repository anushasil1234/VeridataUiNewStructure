import {
    ...
    const scanDLFromWebcam = async () => {
      if (webcamRef.current) {
        ...
        try {
          ...
          // Match common DL formats (like MH12 20110000000 or DL-0420110149646)
          const extractedDL = text.match(/[A-Z]{2}[-\s]?\d{2}[-\s]?\d{4,11}/);
  
          if (extractedDL && extractedDL[0]) {
            const formattedDL = extractedDL[0].replace(/[-\s]/g, '');
            setDrivingLicense(formattedDL);
            showSuccessMessage("Driving License number detected successfully.");
          } else {
            showErrorMessage("Could not detect a valid Driving License number.");
          }
        } catch (err) {
          console.error("OCR error:", err);
          showErrorMessage("Failed to scan the card.");
        } finally {
          setIsScanning(false);
        }
      }
  ...
  {/* Input Method Selection - Only shows if DL is available */}
  {isDLAvailable && (
    <Box sx={{ marginBottom: "20px" }}>
      <Typography sx={{ ...headingType1, lineHeight: "2.4375em", marginLeft: '34px' }}>
        Choose input method:
      ...
      >
        <Box sx={{ display: "flex", alignItems: "center", paddingLeft: "20px" }}>
          ...
          <FormControlLabel value="scan" control={<Radio />} label="Scan with Webcam" />
        </Box>
      </RadioGroup>
    </Box>
  )}
              ...
              <GridRow>
                {inputMethod === "manual" ? (
                  ...
                  <Grid item xs={12} md={6} sx={{ paddingLeft: "0px !important" }}>
                    ...
                    />
                    <Button
                      sx={{ ...submitBtnStyle, marginTop: "10px" }}
                      variant="contained"
                      onClick={scanDLFromWebcam}
                      disabled={isScanning}
                    >
