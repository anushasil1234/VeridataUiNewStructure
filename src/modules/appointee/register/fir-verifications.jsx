...
const FIRVerification = ({
  ...
  const { userId, appointeeId, userCode, candidateId, userName } =
    loggedInData[0];
      ...
      if (value.length <= 10) {
        ...
        if (upperCaseValue.length === 10) {
          if (panRegex.test(upperCaseValue)) {
            ...
            showErrorMessage(
              "Invalid PAN number format. Please enter a valid PAN."
            );
          }
        } else {
          setPanNumberError(false);
      ...
      <GridRow>
        ...
        >
          <TextInput
            ...
            value={
              firstPageForm?.dateOfBirth
                ? DDMMYYYY(firstPageForm?.dateOfBirth)
                : null
            }
            disabled={true}
          />
