...
import { Comment, ThumbDown, Add, ThumbUp } from '@mui/icons-material';
import {
  _addFabStyle, actionIconListStylesx,
  actionIconStyle, cardStyle2,
  floatingIconListStyle,
  gridContainerStyle, listHeadingConteinerStyle,
  listHeadingStyle,
  rightMostBtnStyle,
...
import {
  NA,
  defaultVerificationTypeList, epfFileCategoryTypeAlias, appointeerejetionConfirmationMsg,
  remarksEmptyMsg,
  defaultDropdownValue,
  remarksissuemessage,
  approveConfirmation,
  manuallyVerificationInfo
    ...
    <Box sx={{ ...gridContainerStyle, paddingTop: '-5px', paddingBottom: '0px', ...sx }}>
      <Grid container spacing={1}>
        <Grid item xs={12} md={12} letterSpacing={10}>
          <Box sx={cardStyle2}>{children}</Box>
        </Grid>
      </Grid>
    </Box>
  );
...
let ManualverifiedViewDetails = ({ details, closeModel }) => {
  ...
  const handleChangeVerificationType = async (currentValue, name, _uploadedFileData) => {
    if (currentValue !== 'none') {
      const isSelectedItemDisabled = verificationTypeList.find(
        ({ isDisabled, value }) => value === currentValue && isDisabled === true,
      );
      if (isSelectedItemDisabled) {
        return;
      }
  
      const selectedVerificationType = defaultVerificationTypeList.find(
      ...
      setVerificationCategoryList([]);
      setVerificationQuestionSet([]); // 🔁 Clear old questions
      ...
      const { verificationCategoryList } = filterDocVerificationList({
        ...
        uploadedFileData: _uploadedFileData ?? uploadedFileData,
      });
  ...
  const handleCategoryChange = async (value) => {
    if (value !== defaultDropdownValue) {
      ...
      setSelectedFiles([]);
      const { files } = filterDocVerificationList({
  ...
  const setUploadedFileDataResponse = async (defaultVerificationType) => {
    ...
    if (response) {
      const _uploadedFileData = response?.responseInfos;
      setUploadedFileData(response?.responseInfos);
  ...
  const initializeVerificationData = async () => {
    ...
    if (response) {
      ...
      const { verificationTypeList } = createVerificationTypeList(
        defaultVerificationTypeList,
  ...
  const handleClose = async () => {
    ...
    if (isVarified === false) {
      _contetntText = (
        <>
          ...
          <Typography sx={{ ...subHeadingContentTextStyle, marginTop: '20px' }}>
            {' '}
    ...
    openConfirmationYesNoModal(closeConfirmationModelContent, handleYes, handleNo);
  };

    useEffect(() => {
      const initialQuestions = verificationQuestionSet.filter(
        (q) => q.fieldCode === '014' || q.fieldCode === '015',
      );
      const initialIds = initialQuestions.map((q) => q.questionId);
          ...
          <Grid item xs={12} md={12} lg={12} letterSpacing={2}>
            ...
            <Grid item xs={12}>
              <Stack direction={{ xs: 'column', sm: 'row' }}>
                ...
                <PersonalInformation
                  fieldName={'Physically Handicapped'}
      ...
      <ManualVerifiedPageSectionContainer sx={{ marginTop: '1rem' }}>
        ...
        {verificationCategoryList && verificationCategoryList.length > 0 ? (
          <FiledetailsSection
            ...
            verificationOnChange={handleAnswer}
...
const UnWrappedManualVerifiedView = (props) => {
  return (
    <FullScreenModel
      ...
      fullScreen={true}
      content={
        <ManualverifiedViewDetails
          details={props.appointeePersonalDetails}
