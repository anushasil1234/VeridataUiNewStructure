...
export const ReuploadForm = () => {
  ...
  const { fileTypeList } = dropdownList && dropdownList.length > 0 && dropdownList[0];
  ...
  const handlePostFileReupload = async () => {
    const fileUploaded = MergeWithUniqueKey({
      smallerArray: uploadedFile,
      wholeArrayList: updatedFilesUnfiltered,
      uniqueKey: 'uploadTypeAlias',
    });
    let payLoad = {
    ...
    if (response) {
      ...
      openInfoModel(docResubmissionSuccessContent, () =>
        navigateTo(toDashboard, { state: { status: 'Submited' } }),
      );
    }
  };
  ...
  const setAppointeeDetails = async (appointeeId) => {
    ...
    if (response) {
      let { fileUploaded, isFnameVarified, isUanVarified, memberName } = response.responseInfo;
      const {
      ...
      const { upDatedFileUploaded } = createFileUploadedData({
        fileUploaded,
        verificationFieldModal,
      });
      const { upDatedFileUploaded: updatedFilesUnfiltered } = createFileUploadedData({
        fileUploaded,
      });
      ...
      const stepsList = createReuploadStepSequience({
        isFathersNameVarified: isFnameVarified,
        isUanVarified: isUanVarified,
      });
            ...
            {process.env.REACT_APP_VARIABLE_CERITIFICATE_10TH === 'true' && (
              ...
              >
                ...
                <Grid
                  ...
                  xs={12}
                  md={6}
                  sx={{ paddingLeft: { xs: '0px !important', md: '20px!important' } }}
                >
                  <Typography
                    sx={{
                      ...lable1CopyStyle,
